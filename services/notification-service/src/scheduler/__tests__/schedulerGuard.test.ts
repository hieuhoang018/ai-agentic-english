...
describe('withScheduledReminder', () => {
  ...
  it('calls handler with userId and context when time matches and not yet sent', async () => {
    ...
    await withScheduledReminder('test-reminder', now, prisma, userServiceClient, reminderContextClient, novuClient, webPushSender, handler);

    expect(handler).toHaveBeenCalledWith('user_abc', context, novuClient, webPushSender);
