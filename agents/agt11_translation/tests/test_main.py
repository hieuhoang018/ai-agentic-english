...
def test_get_zone_endpoint():
    ...
    resp = client.get("/zone/user_abc", headers=auth_header("user_abc"))
...
def test_get_zone_endpoint_requires_bearer_token():
    resp = client.get("/zone/user_abc")
    assert resp.status_code == 401


def test_get_zone_endpoint_rejects_mismatched_user():
    resp = client.get("/zone/user_abc", headers=auth_header("someone-else"))
    assert resp.status_code == 403


def test_translate_for_frontend_requires_bearer_token():
    resp = client.post("/translate/user_abc", json={"content": "Hello", "session_type": "exercise"})
    assert resp.status_code == 401


def test_translate_for_frontend_rejects_mismatched_user():
    resp = client.post(
        "/translate/user_abc",
        json={"content": "Hello", "session_type": "exercise"},
...
@respx.mock
def test_translate_for_frontend_accepts_matching_user():
    respx.get(AGT01_PROFILE_URL).mock(
        return_value=httpx.Response(200, json={"irt_theta": {"R": 0.0}})
    )
    ...
    resp = client.post(
        "/translate/user_abc",
        json={"content": "Use present perfect for completed actions.", "session_type": "exercise"},
        headers=auth_header("user_abc"),
    )
