    ...
    <div>
      ...
      <div className="space-y-4">
        {assessmentQuestion.options.map((option, index) => (
          <button key={option} onClick={() => setAnswer(option)} className={`flex h-16 w-full items-center gap-4 rounded-lg border px-5 text-left ${answer === option ? 'border-primary bg-blue-50' : 'border-outline-variant bg-white'}`}>
            <span className="flex h-8 w-8 items-center justify-center rounded-full border border-outline">{String.fromCharCode(65 + index)}</span>
            {option}
          </button>
        ))}
      </div>
