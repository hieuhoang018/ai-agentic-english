  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      {skills.map((skill) => {
        ...
        return (
          <button key={skill.id} onClick={() => updateProfile({ prioritySkills: active ? selected.filter((item) => item !== skill.id) : [...selected, skill.id] })} className={`rounded-lg border p-5 text-center ${active ? 'border-primary bg-violet-50 text-tertiary' : 'border-outline-variant bg-white text-on-surface-variant'}`}>
