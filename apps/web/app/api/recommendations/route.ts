...
export async function GET() {
  ...
  try {
    const recommendations = await apiFetch<RecommendationItem[]>(`/recommendations/${userId}`, { token });
    ...
    return NextResponse.json({ message: 'Unable to load recommendations right now.' }, { status: 502 });
  }
}
