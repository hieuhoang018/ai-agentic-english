...
export async function GET() {
  ...
  try {
    const profile = await apiFetch<ProfileSummaryResponse>(`/profile/${userId}`, { token });
    ...
    return NextResponse.json({ message: 'Unable to load your profile right now.' }, { status: 502 });
  }
}
