...
export default function SideMenu() {
  ...
  return (
    <nav className="bg-surface-container-low dark:bg-surface-container-highest h-screen w-64 fixed left-0 top-0 hidden md:flex flex-col border-r border-outline-variant dark:border-outline z-50 justify-between">
      <div>
        ...
        <div className="overflow-y-auto py-stack-md px-3 space-y-2">
          ...
          <Link href="/main/review-center" className={`${base} ${isReview ? active : inactive}`} aria-current={isReview ? 'page' : undefined}>
      ...
      <div className="py-4 px-3 space-y-2 border-t border-outline-variant/30 mt-auto">
        <Link className="flex items-center px-4 py-2 rounded-lg text-on-surface-variant dark:text-surface-dim hover:text-primary hover:bg-surface-container-high dark:hover:bg-surface-variant transition-colors duration-200" href="/main/help">
          <span className="material-symbols-outlined mr-3 text-sm">help</span>
          <span className="text-sm">Trợ giúp</span>
        </Link>
        <Link className="flex items-center px-4 py-2 rounded-lg text-on-surface-variant dark:text-surface-dim hover:text-primary hover:bg-surface-container-high dark:hover:bg-surface-variant transition-colors duration-200" href="/main/about">
