...
const RATING_OPTIONS: { label: string; quality: number; tone: string }[] = [
  { label: 'Quên', quality: 0, tone: 'bg-error/10 text-error hover:bg-error/20' },
  { label: 'Khó', quality: 2, tone: 'bg-orange-100 text-orange-700 hover:bg-orange-200' },
  { label: 'Tốt', quality: 4, tone: 'bg-secondary-container/40 text-secondary hover:bg-secondary-container/60' },
  { label: 'Dễ', quality: 5, tone: 'bg-primary-container/30 text-primary hover:bg-primary-container/50' },
]
    ...
    <div className="mx-auto max-w-3xl pt-6 sm:pt-16">
      ...
      {state.status === 'error' && (
        <div className="rounded-lg border border-dashed border-outline-variant bg-white p-10 text-center">
          ...
          <h1 className="mt-4 text-2xl font-bold text-on-surface">Không thể tải dữ liệu ôn tập</h1>
      ...
      {state.status === 'success' && state.items.length === 0 && (
        <div className="rounded-lg border border-dashed border-outline-variant bg-white p-10 text-center">
          ...
          <h1 className="mt-4 text-2xl font-bold text-on-surface">Không có từ nào đến hạn</h1>
          <p className="mt-2 text-on-surface-variant">Bạn đã ôn tập hết! Quay lại sau nhé.</p>
        </div>
      )}
      ...
      {state.status === 'success' && state.items.length > 0 && index >= state.items.length && (
        <div className="rounded-lg border border-dashed border-outline-variant bg-white p-10 text-center">
          ...
          <h1 className="mt-4 text-2xl font-bold text-on-surface">Hoàn thành!</h1>
          <p className="mt-2 text-on-surface-variant">Bạn đã ôn tập {ratedCount} từ hôm nay.</p>
        </div>
      )}
      ...
      {state.status === 'success' && state.items.length > 0 && index < state.items.length && (
        <>
          <button
            ...
            className="flex min-h-[320px] w-full flex-col items-center justify-center rounded-lg border border-outline-variant bg-white p-5 text-center shadow-[0_12px_36px_-24px_rgba(15,23,42,0.55)] sm:p-8"
          >
