  ...
  return (
    <div className="pb-8">
      ...
      <section className="mb-8">
        <h2 className="mb-5 flex items-center gap-2 text-2xl font-bold text-on-surface">
        ...
        {examples.length > 0 ? (
          <div className="space-y-4">
            {examples.map((example) => (
              <div
                key={example.id}
                className="rounded-lg border border-outline-variant border-l-4 border-l-primary bg-white p-4"
              >
                <p className="font-semibold">{example.text}</p>
                {example.note ? <p className="mt-1 italic text-on-surface-variant">{example.note}</p> : null}
              </div>
            ))}
          </div>
        ) : (
          <div className="rounded-lg border border-dashed border-outline-variant bg-white p-6 text-on-surface-variant">
      ...
      {lesson.source || lesson.license ? (
        <section className="rounded-lg border border-outline-variant bg-surface-container-lowest p-5 text-sm text-on-surface-variant">
