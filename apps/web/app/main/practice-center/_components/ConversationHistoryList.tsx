  ...
  return (
    <div>
      <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <h1 className="text-4xl font-bold text-on-surface">Lịch sử hội thoại</h1>
      ...
      <div className="space-y-10">
        {Object.entries(grouped).map(([dateLabel, rows]) => (
          <section key={dateLabel}>
            ...
            <div className="space-y-4 border-l border-outline-variant pl-8">
              {rows.map((conversation) => (
                <article key={conversation.id} className="relative rounded-lg border border-outline-variant/60 bg-surface-container-lowest p-4 shadow-[0_8px_24px_-20px_rgba(15,23,42,0.45)]">
                  <span className="absolute -left-[38px] top-7 h-3 w-3 rounded-full border-4 border-background bg-primary" />
