...
function getStatusClassName(status: SpeakingRealtimeStatus) {
  if (status === 'error') return 'text-error dark:text-red-400'
  if (status === 'recording') return 'text-tertiary'
  if (status === 'ending' || status === 'ended') return 'text-on-surface-variant dark:text-surface-dim'
...
export default function SpeakingChat({ session }: SpeakingChatProps) {
  ...
  return (
    <section className="flex min-h-[520px] flex-col overflow-hidden rounded-lg border border-outline-variant/70 bg-surface-container-lowest dark:border-outline/70 dark:bg-surface-dark sm:min-h-[620px]">
      <header className="flex flex-col gap-4 border-b border-outline-variant/70 p-4 dark:border-outline/70 sm:flex-row sm:items-center sm:justify-between sm:p-5">
        <div className="flex items-center gap-4">
          <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-primary text-white">
            <span className="material-symbols-outlined">smart_toy</span>
          </div>
          <div>
            <h2 className="text-xl font-bold text-on-surface dark:text-on-primary sm:text-2xl">Wise Mentor AI</h2>
        ...
        <button
          ...
          className="flex h-10 items-center justify-center gap-2 rounded-lg border border-outline-variant bg-white px-4 text-sm font-semibold text-on-surface transition-colors hover:bg-surface-container disabled:cursor-not-allowed disabled:opacity-50 dark:border-outline dark:bg-surface-dark dark:text-on-primary dark:hover:bg-surface-dark-high"
        >
      ...
      <div className="flex-1 space-y-6 overflow-y-auto bg-[#f7f8fd] p-4 dark:bg-surface-dark-high sm:p-6">
        <div className="flex justify-center">
          <span className="rounded-full bg-surface-container-high px-4 py-1 text-xs text-on-surface-variant dark:bg-surface-dark dark:text-surface-dim">
        ...
        {messages.map((message) => {
          ...
          return (
            <div key={message.id} className={`flex items-start gap-3 ${isUser ? 'justify-end' : 'justify-start'}`}>
              ...
              <div className={`max-w-[86%] sm:max-w-[78%] ${isUser ? 'text-right' : ''}`}>
                <div
                  className={`min-h-7 rounded-lg px-4 py-3 text-left leading-7 shadow-sm ${
                    isUser ? 'bg-primary text-white' : 'border border-outline-variant bg-white text-on-surface dark:border-outline dark:bg-surface-dark-high dark:text-on-primary'
                  }`}
                >
                  {messageContent}
                </div>
                <div
                  className={`mt-1 flex items-center gap-2 text-xs ${
                    isUser ? 'justify-end text-secondary' : 'text-on-surface-variant dark:text-surface-dim'
                  }`}
                >
                ...
                {isUser && (message.mockFeedback || detectedIssues > 0) ? (
                  <div className="mt-2 space-y-2 text-left">
                    {message.mockFeedback ? (
                      <div className="rounded-lg border border-outline-variant bg-white px-3 py-2 text-xs leading-5 text-on-surface-variant dark:border-outline dark:bg-surface-dark-high dark:text-surface-dim">
                        {message.mockFeedback}
                      </div>
                    ) : null}
                    {detectedIssues > 0 ? (
              ...
              {isUser ? (
                <div className="h-9 w-9 shrink-0 overflow-hidden rounded-full bg-surface-container dark:bg-surface-dark-high">
                  <div className="flex h-full w-full items-center justify-center text-sm font-bold text-primary dark:text-primary-fixed-dim">A</div>
                </div>
              ) : null}
            </div>
          )
        ...
        {messages.length === 0 ? (
          <div className="flex justify-start">
            <div className="max-w-[86%] rounded-lg border border-outline-variant bg-white px-4 py-3 leading-7 text-on-surface-variant shadow-sm dark:border-outline dark:bg-surface-dark-high dark:text-surface-dim sm:max-w-[78%]">
        ...
        {isBusy ? (
          <div className="flex justify-start">
            <div className="rounded-lg border border-outline-variant bg-white px-5 py-3 text-on-surface shadow-sm dark:border-outline dark:bg-surface-dark-high dark:text-on-primary">
      ...
      <footer className="border-t border-outline-variant/70 p-4 dark:border-outline/70">
        <form onSubmit={handleSubmit} className="flex items-center gap-2 sm:gap-3">
          <input
            value={draft}
            onChange={(event) => setDraft(event.target.value)}
            disabled={isBusy || isSessionClosed || isRecording}
            className="h-12 min-w-0 flex-1 rounded-lg border border-outline-variant bg-white px-4 text-base outline-none focus:border-primary disabled:cursor-not-allowed disabled:opacity-60 dark:border-outline dark:bg-surface-dark-high dark:text-on-primary"
        ...
        <div className="mt-3 flex items-center justify-between text-sm text-on-surface-variant dark:text-surface-dim">
