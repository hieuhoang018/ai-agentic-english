        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-gutter w-full">
          ...
          <div className="bg-surface-container-lowest dark:bg-surface-container-high rounded-lg p-card-padding shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] border border-outline-variant/20 dark:border-outline/20 h-full">
            <div className="flex justify-between items-center mb-6 gap-4">
              <h3 className="text-2xl font-semibold text-on-surface dark:text-on-primary">Nhiệm vụ hằng ngày</h3>
              <span className="text-sm bg-surface-container dark:bg-surface-variant px-2 py-1 rounded-md text-on-surface-variant dark:text-surface-dim shrink-0">2/4 hoàn thành</span>
            </div>
            <div className="space-y-3">
              {[
                ['Học 5 từ vựng mới', 'Chủ đề: Công nghệ'],
                ['Hoàn thành 1 bài đọc ngắn', 'Chủ đề: Môi trường'],
              ].map(([title, detail]) => (
                <div key={title} className="flex items-start gap-3 p-3 rounded-lg bg-surface-container-low/50 dark:bg-surface-variant/50 opacity-70">
                  <span className="material-symbols-outlined text-secondary mt-0.5">check_circle</span>
                  <div>
                    <h4 className="text-base line-through text-on-surface-variant dark:text-surface-dim">{title}</h4>
                    <p className="text-sm text-on-surface-variant/80 dark:text-surface-dim/80 mt-1">{detail}</p>
                  </div>
                </div>
              ))}
              {[
                ...
                <Link key={title} href={href} className="flex items-start gap-3 p-3 rounded-lg bg-surface-container dark:bg-surface-variant border-l-2 border-primary cursor-pointer hover:bg-surface-container-high dark:hover:bg-surface-container transition-colors group">
                  ...
                  <div className="flex-1">
                    <h4 className="text-base text-on-surface dark:text-on-primary group-hover:text-primary transition-colors">{title}</h4>
                    <p className="text-sm text-on-surface-variant dark:text-surface-dim mt-1">{detail}</p>
                  </div>
